package org.sang.trade.utils;

import org.sang.trade.config.WxRequestParam;
import org.sang.trade.service.OrderServiceImpl;
...
public class WxPayUtil {
    ...
    public static String getCodeUrl(WxRequestParam param) {
        String codeUrl = "";
        try {
            String result = sendRequest("https://api.mch.weixin.qq.com/pay/unifiedorder", new String(requestParam(param).getBytes(), "utf-8"));
    ...
    //查询定单状态
    public boolean queryOrderState(WxRequestParam param) {
        ...
        try {
            ...
            if ("SUCCESS".equals(trade_state)) {
                String id = (String) map.get("transaction_id");
                String no = (String) map.get("out_trade_no");
                orderService.paySuccess(no, 2, id);
                return true;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static void main(String[] args) {
        ...
        param.setOut_trade_no("D100000120171130164859d3b09e");
        WxPayUtil wxPayUtil = new WxPayUtil();
        System.out.println(wxPayUtil.queryOrderState(param));
    }
